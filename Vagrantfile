...
if options['github_token'].nil? || options['github_token'].to_s.length != 40
  puts "You must place REAL GitHub token into configuration:\n/coukraine/vagrant/config/vagrant-local.yml"
  exit
end
...
Vagrant.configure(2) do |config|
  ...
  # sync: folder 'coukraine' (host machine) -> folder '/app' (guest machine)
  config.vm.synced_folder './', '/var/www/coukraine', owner: 'vagrant', group: 'vagrant'
